...
public class BasicHealingJutsu extends CommonScrollBase{
    public BasicHealingJutsu() {
    ...
    public void useItem(World worldIn, PlayerEntity playerIn, Hand handIn) {
        ...
        playerIn.addPotionEffect(new EffectInstance(Effects.REGENERATION, 300, 1));
    }

    @Override
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        tooltip.add(new StringTextComponent("Regenerate health"));
        if(!KeyboardUtil.isHoldingShift())
            tooltip.add(new StringTextComponent("\u00A7c" + "Hold Shift For More Info"));
        else {
            tooltip.add(new StringTextComponent("\u00A7b" + "Effects"));
            tooltip.add(new StringTextComponent( "Regeneration 2"));
        }
    }
}
